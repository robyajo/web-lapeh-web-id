...
export default async function Authors() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 mb-48 max-w-380 w-full mx-auto border border-black border-collapse">
      {data.map((article) => (
        <article
        ...
        >
          <Link href={`authors/${article.slug}`}>
            <Image
              className="w-37.5 h-37.5 object-cover rounded-full hover:scale-105 transition"
              src={article.avatar}
              alt={article.imgAlt}
              width={150}
              height={150}
